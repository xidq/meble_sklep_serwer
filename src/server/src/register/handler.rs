...
// use sqlite_serv::auth::jwt::load_users_from_json;
...
//     let username = payload.username.trim();
//     let password = payload.password;
//     let pepper = state.pepper_key.clone();
// 
...
//     let new_user = match User::new(0, username.to_string(), UsrPermit::User, &password, &pepper) {
//         Ok(user) => user,
//         Err(_) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({ "error": "Błąd hashowania" }))).into_response(),
//     };
// 
